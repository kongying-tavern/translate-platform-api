...
use _database::functions::models::document::{delete as do_delete, insert as do_insert};
...
pub async fn insert(
    ...
    if let Some(auth) = auth {
        ...
        let ret = do_insert(auth.id, data, file_name_raw)
            .await
            .map_err(|err| (StatusCode::INTERNAL_SERVER_ERROR, err.to_string()))?;
        Ok(Json(ret))
    } else {
